...
#nullable enable
        private TpSlItems2? CatchOrders(Trade trade)
        {
            ...
            if (trade.Comment != null)
            {
                ...
                if (splitted is KeyValuePair<string, OrderTypeSubcomment> parsedComment && this.Items.Any(x => x.Id == parsedComment.Key))
                {
                    ...
                    if (result == null)
                        Core.Instance.Loggers.Log("[ManagerDue] More Then One Item With Same Id", LoggingLevel.Error);
                    ...
                    else
                    {
                        switch (trade.PositionImpactType)
                        {
                            ...
                            default:
                                ...
                                {
                                    Core.Instance.Loggers.Log("[ManagerDue] " + $"Undefined PositionImpactType Error: {ex.Message}", LoggingLevel.Error);
                                    throw;
                                }
                                break;
                        }
                    ...
                    else
                    {
                        switch (parsedComment.Value)
                        {
                            ...
                            case OrderTypeSubcomment.Entry:
                                ...
                                break;
                            default:
                                Core.Instance.Loggers.Log("[ManagerDue] Unknown Subcomment Type", LoggingLevel.Error);
                                break;
                        }
                    }
        ...
        public override void CreateItem(string comment)
        {
            ...
            //TODO: Core.Instance.Loggers.Log("[ManagerDue] " + $"? Creato nuovo SlTpItems con ID: {comment}", LoggingLevel.System);
            }
            else
            {
            //TODO: Core.Instance.Loggers.Log("[ManagerDue] " + $"?? Item con ID {comment} già esistente, non ricreato.", LoggingLevel.Error);
            }
        }
            ...
            lock (_lockObj)
            {
                ...
                try
                {
                    if (!ClosedItems.Any(x => x.Id == item.Id))
                    {
                        ...
                        Items.Remove(item);
                        item.ItemClosed -= Item_ItemClosed;
                    //TODO: Core.Instance.Loggers.Log("[ManagerDue] " + $"? Item chiuso e spostato in ClosedItems: {item.Id}", LoggingLevel.System);
                    }
                    else
                    {
                        //TODO:Core.Instance.Loggers.Log("[ManagerDue] " + $"?? Tentativo di chiudere un item già chiuso: {item.Id}", LoggingLevel.Warning);
                    }
                }
                catch (Exception ex)
                {
                    Core.Instance.Loggers.Log("[ManagerDue] " + $"Errore durante la chiusura dell'item: {ex.Message}", LoggingLevel.Error);
                    throw;
                }
